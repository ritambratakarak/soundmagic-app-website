            ...
            <TouchableOpacity onPress={() => changeTab(i, data._id)} style={[{ justifyContent: "center", height: 40, alignItems: 'center', margin: 5, marginRight: WIDTH * 0.040 }]}>
              {activeTab == i ?
                <View style={{ width: "100%", height: "100%", alignItems: "center", justifyContent: "center", }}>
                  <Text style={[{ textAlign: 'center', fontSize: FONT.SIZE.LARGE, color: COLORS.PRIMARY, marginVertical: 5 }, activeTab == i ? { fontWeight: "bold", fontFamily: FONT.FAMILY.BOLD } : { fontWeight: "200", color: "#fff", }]}>{data.name}</Text>
                  <View style={{ height: 2, width: "100%", backgroundColor: "#fff" }} />
                </View>
                :
                <Text style={[{ textAlign: 'center', fontSize: FONT.SIZE.LARGE, fontFamily: FONT.FAMILY.BOLD }, { color: COLORS.GRAY }]}>{data.name}</Text>
              }
            </TouchableOpacity>
          )
        }) : null}
