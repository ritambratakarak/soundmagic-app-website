...
const hitSlop = { top: 25, bottom: 14, left: 14, right: 5 }
    ...
    <>
      <View
        ...
        <View style={kContainerStyle}>
          <View style={{ paddingRight: 20, flexGrow: 1, width:"100%" }}>
            {isMulti ? (
              <FlatList
                data={selectedValues}
              ...
              <TouchableOpacity hitSlop={hitSlop} onPress={onPressShowOptions()} style={{width:"100%", backgroundColor:"gray"}}>
                <Text style={kSelectedItemStyle()}>{value.item || inputPlaceholder || label}</Text>
              </TouchableOpacity>
            )}
          </View>
          <TouchableOpacity onPress={onPressShowOptions()} hitSlop={hitSlop} style={{right:"3%", position:"absolute", top:10}}>
            {selectIcon ? selectIcon : <AntDesign name={showOptions ? 'up' : 'down'} color={"#000"} size={20} />}
          </TouchableOpacity>
        </View>
        {/* Options wrapper */}
        {showOptions && (
