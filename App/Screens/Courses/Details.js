...
function Details() {
  ...
  const EnrollNow = async () => {
    ...
    Network('/enroll-course', 'post', submitData)
      .then(async (res) => {
        console.log(res);
        setLoading(false);
        if (res.response_code === 200) {
  ...
  const getCoursesDetails = async () => {
    ...
    const data = JSON.parse(userdata);
    const authtoken = data.authtoken;
    Network(`/get-course?page=${1}&limit=${100}&_id=${alldata._id}`, 'get', {
      authtoken,
    })
      .then(async (res) => {
        setLoading(false);
        if (res.response_code === 200) {
          const alldata = res.response_data.docs;
          console.log(
            'courses data',
            alldata.length != 0 ? res.response_data.docs[0] : '',
          );
          setalldata(alldata.length != 0 ? res.response_data.docs[0] : '');
          // Toast.show(res.response_message);
        } else if (res.response_code === 4000) {
          Toast.show(res.response_message);
          await AsyncStorage.removeItem('@user');
